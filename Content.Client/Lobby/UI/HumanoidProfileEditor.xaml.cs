...
using Content.Client._radiant.Lobby.UI; // Radiant Sector
...
namespace Content.Client.Lobby.UI
{
    ...
    public sealed partial class HumanoidProfileEditor : BoxContainer
    {
        ...
        // Radiant Sector: detailed species selector opened from the appearance tab.
        private SpeciesSelectionWindow? _speciesSelectionWindow;
            ...
            MarkingManager markings)
        {
            ...
            // Radiant Sector: darken the working surface of the character-editor tabs without affecting controls.
            TabContainer.PanelStyleBoxOverride = new StyleBoxFlat
            {
                BackgroundColor = Robust.Shared.Maths.Color.FromHex("#252A30")
            };
            ...
            SpeciesPickerButton.OnPressed += _ => OpenSpeciesSelectionWindow(); // Radiant Sector
            ...
            WidthContainer.Visible = !prototype.UseUniformScale;
            ...
            _heightSlider.OnValueChanged += args =>
            {
                ...
                prototype = _speciesList.Find(x => x.ID == Profile.Species) ?? _speciesList.First();
                ...
                if (prototype.UseUniformScale)
                {
                    // Единый размер: одновременно меняем Height и Width.
            ...
            CHeightReset.OnPressed += _ =>
            {
                ...
                if (prototype.UseUniformScale)
                {
                    SetProfileWidth(prototype.DefaultHeight);
                }
            ...
            CWidthLabel.Text = Loc.GetString(
            ...
            _widthSlider.OnValueChanged += args =>
            {
                ...
                // В режиме общего размера Width slider скрыт,
                // поэтому здесь ничего специального делать не нужно.
                if (prototype.UseUniformScale)
                    return;
        ...
        private void UpdateHeightLabel(SpeciesPrototype species)
        {
            if (Profile is null)
                return;

            if (species.UseUniformScale)
            {
                var size = MathF.Round(
                    species.AverageHeight * _heightSlider.Value);

                CHeightLabel.Text = Loc.GetString(
                    "humanoid-profile-editor-size-label",
                    ("size", (int)size));

                return;
            }

            var height = MathF.Round(
                species.AverageHeight * _heightSlider.Value);
        ...
        public void RefreshTraits()
        {
            ...
            foreach (var trait in traits)
            {
                ...
                // Radiant Sector: do not offer native-language limitations to species without a native language.
                if (Profile != null &&
                    (trait.ID == "NativeLanguageUnfamiliar" || trait.ID == "NativeLanguageOnly") &&
                    !HumanoidCharacterProfile.HasNativeLanguage(Profile.Species))
                    continue;
        ...
        public void RefreshSpecies()
        {
            ...
            for (var i = 0; i < _species.Count; i++)
            {
                ...
                if (Profile?.Species.Equals(_species[i].ID) == true)
                {
                    ...
                    SpeciesPickerButton.Text = name; // Radiant Sector
                }
            }
        ...
        // Radiant Sector: replaces the unreadable species drop-down with a preview and lore card.
        private void OpenSpeciesSelectionWindow()
        {
            if (Profile == null)
                return;

            _speciesSelectionWindow?.Dispose();
            _speciesSelectionWindow = new SpeciesSelectionWindow(Profile, _prototypeManager, _entManager, _controller);
            _speciesSelectionWindow.OnSpeciesChosen += species =>
            {
                SetSpecies(species);
                UpdateHairPickers();
                OnSkinColorOnValueChanged();
                RefreshSpecies();
                _speciesSelectionWindow?.Dispose();
                _speciesSelectionWindow = null;
            };
            _speciesSelectionWindow.OnClose += () => _speciesSelectionWindow = null;
            _speciesSelectionWindow.OpenCentered();
        }
        ...
        private void UpdateERPStatus()
        {
            if (_erpStatus != null)
            {
                _erpStatus.SelectId((int) (Profile?.ERPStatus ?? EnumERPStatus.HALF));
            }
        }
        ...
        private void UpdateWidthControls()
        {
            ...
            var width = MathF.Round(
                species.AverageWidth * Profile.Width);
