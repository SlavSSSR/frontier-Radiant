ent-VendingMachineCuddlyCritterVend = ПлюшкоВенд
    .desc = Добро пожаловать в царство плюшевых чудес! Этот автомат — настоящая находка для любителей мягких игрушек, способный подарить вам уют и тепло в любое время.
ent-VendingMachineAstroVend = АстроВенд
    .desc = Ваш надежный спутник в космосе! Здесь вы найдете все необходимое снаряжение для покорения галактики, чтобы вы всегда были готовы к новым приключениям.
ent-VendingMachineFlatpackVend = Упак-О-Мат
    .desc = Крутые технологии в компактной упаковке! Получите все, что нужно для ваших космических путешествий, с этим универсальным автоматом.
ent-VendingMachineExpeditionaryFlatpackVend = Экспедиционный Упак-О-Мат
    .desc = Современные технологии для самых смелых экспедиций! Исследуйте неизведанные планеты с лучшим снаряжением для первооткрывателей.
ent-LessLethalVendingMachine = Травма-То-Венд
    .desc = Обеспечиваем ограничение насилия с 08-года!
ent-VendingMachineSyndieContraband = КонтраВенд
    .desc = Окунитесь в мир запретных товаров! Только для тех, кто не боится нарушить правила ради эксклюзивных шаттлов и уникальных артефактов. Осторожно: находитесь под наблюдением!
ent-VendingMachineBountyVend = КонтрактВенд
    .desc = От снаряжения до контрактов — всё в одном месте! Обновите свой арсенал и получите лучшее предложение для выполнения любых заданий.
ent-VendingMachineArcadia = АркадиаМаг
    .desc = Откройте для себя моду из других измерений! Этот автомат предлагает одежду, которая подчеркнет вашу уникальность и неординарность по самым доступным ценам.
ent-BaseCiviMedVend = МедВенд
    .desc = Обезопасьте себя и других! Наша продукция делает насилие безопасным, предлагая лучшие решения для самообороны.
ent-NonLethalVendingMachine = БезопасныйВенд
    .desc = Безопасное снаряжение, на которое можно положиться. Только не целитесь в глаза — даже у самых безопасных игрушек есть свои правила!
ent-VendingMachineAutoTuneVend = МузВенд
    .desc = Пробудите музыкального гения! Настройте свою волну с МузВендом и взорвите космос своими ритмами. Пусть ваша музыка звучит громче, чем звезды!
ent-VendingMachinePottedPlantVend = Трав-О-Маг
    .desc = Зелёные друзья всегда под рукой! Купите растение в горшке и обзаведитесь идеальным напарником, который не задает лишних вопросов и добавляет уют в любой уголок космоса.
ent-VendingMachineNfsdDrobe = ДВБдроб
    .desc = Элитная экипировка для сотрудников ДВБ! Будьте в авангарде моды и безопасности с лучшей формой и снаряжением для защитников порядка.
ent-VendingMachineYarrrDrobe = ЙаррДроб
    .desc = Для настоящих пиратов и искателей приключений! Этот автомат полон сокровищ для тех, кто любит лёгкую добычу и жизнь без правил.
ent-VendingMachineNfsdTech = ДВБФтех
    .desc = Передовые технологии для лучших из лучших. Откройте для себя мощные инструменты и устройства, которые помогут вам оставаться на шаг впереди.
ent-VendingMachineCartNfsd = { ent-VendingMachineCart }
    .suffix = ДВБ
    .desc = { ent-VendingMachineCart.desc }
ent-VendingMachineClothingPunk = ПанкоМат
    .desc = Одежда с характером! Этот автомат для тех, кто не боится заявить о себе. Стань панком и выделись из толпы с уникальными вещами.
ent-VendingMachineBountyVendPunk = БонанзоМат
    .desc = Весь набор для бунтаря! Этот автомат предлагает контракты и снаряжение для тех, кто готов взорвать мир своим стилем и решимостью.
ent-VendingMachineFuelVend = ТоплиМат
    .desc = Торговый автомат с топливом для двигателей вашего корабля. Пахнет, как будто в воздухе зависла сама молния!
ent-VendingMachineCiviMedVend = МедВенд
    .desc = МедВенд: аптечка на колесах, которая лечит быстрее, чем вы успеете расколоть камень! Шахтёры, берегите себя!
ent-VendingMachineAstroVendPOI = { ent-VendingMachineAstroVend }
    .desc = { ent-VendingMachineAstroVend.desc }
ent-LessLethalVendingMachinePOI = { ent-LessLethalVendingMachine }
    .desc = { ent-LessLethalVendingMachine.desc }
ent-VendingMachineCiviMedPlus = { ent-VendingMachineCiviMedVend }
    .desc = { ent-VendingMachineCiviMedVend.desc }
ent-VendingMachineCiviMed = { ent-VendingMachineCiviMedVend }
    .desc = { ent-VendingMachineCiviMedVend.desc }
ent-VendingMachineAmmoPOI = { ent-VendingMachineAmmo }
    .desc = { ent-VendingMachineAmmo.desc }
ent-VendingMachineEngivendPOI = { ent-VendingMachineEngivend }
    .desc = { ent-VendingMachineEngivend.desc }
ent-VendingMachineVendomatPOI = { ent-VendingMachineVendomat }
    .desc = { ent-VendingMachineVendomat.desc }
ent-VendingMachineYouToolPOI = { ent-VendingMachineYouTool }
    .desc = { ent-VendingMachineYouTool.desc }
ent-VendingMachineSalvagePOI = { ent-VendingMachineSalvage }
    .desc = { ent-VendingMachineSalvage.desc }
ent-VendingMachineTankDispenserEVAPOI = { ent-VendingMachineTankDispenserEVA }
    .desc = { ent-VendingMachineTankDispenserEVA.desc }
ent-NFVendingMachineCart = { ent-VendingMachineSalvage }
    .suffix = Губернатор
    .desc = { ent-VendingMachineSalvage.desc }
ent-NFVendingMachineCartNfsd = { ent-VendingMachineSalvage }
    .suffix = ДВБ
    .desc = { ent-VendingMachineSalvage.desc }
ent-VendingMachineMailVend = ПочтаВенд
    .desc = Озвучьте свои мысли, отправьте письмо сегодня!
ent-VendingMachineValetDrobe = КонсьержВенд
    .desc = Помогайте другим стильно! Одевайтесь по высшему разряду, чтобы получить лучшие советы!
ent-VendingMachinePottedPlantVendPOI = { ent-VendingMachinePottedPlantVend }
    .suffix = POI
    .desc = { ent-VendingMachinePottedPlantVend.desc }
ent-VendingMachineBountyVendPOI = { ent-VendingMachineBountyVend }
    .suffix = POI
    .desc = { ent-VendingMachineBountyVend.desc }
ent-VendingMachineMediDrobePOI = { ent-VendingMachineMediDrobe }
    .suffix = POI
    .desc = { ent-VendingMachineMediDrobe.desc }
ent-VendingMachinePosterPrinter = ПостерПресс
    .desc = Печатай и вешай! Или вешай и печатай?
    .suffix = Плакаты Фронтира
ent-VendingMachinePosterPrinterPOI = { ent-VendingMachinePosterPrinter }
    .desc = { ent-VendingMachinePosterPrinter.desc }
    .suffix = POI, Плакаты Фронтира
ent-VendingMachinePosterPrinterNanotrasen = { ent-VendingMachinePosterPrinter }
    .desc = { ent-VendingMachinePosterPrinter.desc }
    .suffix = Плакаты Nanotrasen
ent-VendingMachinePosterPrinterNanotrasenPOI = { ent-VendingMachinePosterPrinter }
    .desc = { ent-VendingMachinePosterPrinter.desc }
    .suffix = POI, Плакаты Nanotrasen
ent-VendingMachinePosterPrinterSyndicate = { ent-VendingMachinePosterPrinter }
    .desc = { ent-VendingMachinePosterPrinter.desc }
    .suffix = Контрабандные плакаты
ent-VendingMachinePosterPrinterSyndicatePOI = { ent-VendingMachinePosterPrinter }
    .desc = { ent-VendingMachinePosterPrinter.desc }
    .suffix = POI, Контрабандные плакаты
ent-VendingMachinePaintingPrinter = АртПресс
    .desc = Печатай и вешай! Или вешай и печатай?
    .suffix = Картины
ent-VendingMachinePaintingPrinterPOI = { ent-VendingMachinePaintingPrinter }
    .desc = { ent-VendingMachinePaintingPrinter.desc }
    .suffix = POI, Картины
